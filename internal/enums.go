...
type EPackageFileTag uint32

const (
	PACKAGE_FILE_TAG EPackageFileTag = 0x9E2A83C1

	PACKAGE_FILE_TAG_SWAPPED EPackageFileTag = 0xC1832A9E
)

type EUnrealEngineObjectUE4Version uint32

const (
	VER_UE4_OLDEST_LOADABLE_PACKAGE EUnrealEngineObjectUE4Version = iota + 214
...
const (
	// No flags
	PKG_None PackageFlags = 0x00000000

	// Newly created package, not saved yet. In editor only.
	PKG_NewlyCreated PackageFlags = 0x00000001

	// Purely optional for clients.
	PKG_ClientOptional PackageFlags = 0x00000002

	// Only needed on the server side.
	PKG_ServerSideOnly PackageFlags = 0x00000004

	// This package is from "compiled in" classes.
	PKG_CompiledIn PackageFlags = 0x00000010

	// This package was loaded just for the purposes of diffing
	PKG_ForDiffing PackageFlags = 0x00000020

	// This is editor-only package (for example: editor module script package)
	PKG_EditorOnly PackageFlags = 0x00000040

	// Developer module
	PKG_Developer PackageFlags = 0x00000080

	// Loaded only in uncooked builds (i.e. runtime in editor)
	PKG_UncookedOnly PackageFlags = 0x00000100

	// PKG_Unused PackageFlags = 0x00000200
	// PKG_Unused PackageFlags = 0x00000400
	// PKG_Unused PackageFlags = 0x00000800
	// PKG_Unused PackageFlags = 0x00001000

	// Uses unversioned property serialization instead of versioned tagged property serialization
	PKG_UnversionedProperties PackageFlags = 0x00002000

	// Contains map data (UObjects only referenced by a single ULevel) but is stored in a different package
	PKG_ContainsMapData PackageFlags = 0x00004000

	// PKG_Unused PackageFlags = 0x00008000

	// package is currently being compiled
	PKG_Compiling PackageFlags = 0x00010000

	// Set if the package contains a ULevel/ UWorld object
	PKG_ContainsMap PackageFlags = 0x00020000

	// Set if the package contains any data to be gathered by localization
	PKG_RequiresLocalizationGather PackageFlags = 0x00040000

	// PKG_Unused PackageFlags = 0x00080000

	// Set if the package was created for the purpose of PIE
	PKG_PlayInEditor PackageFlags = 0x00100000

	// Package is allowed to contain UClass objects
	PKG_ContainsScript PackageFlags = 0x00200000

	// Editor should not export asset in this package
	PKG_DisallowExport PackageFlags = 0x00400000

	// PKG_Unused PackageFlags = 0x00800000
	// PKG_Unused PackageFlags = 0x01000000
	// PKG_Unused PackageFlags = 0x02000000
	// PKG_Unused PackageFlags = 0x04000000
	// PKG_Unused PackageFlags = 0x08000000

	// This package should resolve dynamic imports from its export at runtime.
	PKG_DynamicImports PackageFlags = 0x10000000

	// This package contains elements that are runtime generated, and may not follow standard loading order rules
	PKG_RuntimeGenerated PackageFlags = 0x20000000

	// This package is reloading in the cooker, try to avoid getting data we will never need. We won't save this package.
	PKG_ReloadingForCooker PackageFlags = 0x40000000

	// Package has editor-only data filtered out
	PKG_FilterEditorOnly PackageFlags = 0x80000000
)

type FCustomVersionContainer int32
